...
const TermsOfPolicy = () => {
  ...
  return (
    <div ref={containerRef} className="section-stars-bg min-h-screen text-white pt-32 pb-24 relative">
      ...
      <div className="max-w-[95vw] lg:max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="parallax-container relative w-full h-[35vh] md:h-[45vh] rounded-[2.5rem] overflow-hidden mb-16 reveal-up shadow-2xl isolate flex flex-col justify-center items-center text-center">
          ...
          <div className="relative z-[2] px-4">
            <h1 className="text-4xl md:text-6xl font-bold tracking-normal text-white drop-shadow-2xl mb-4">
              Terms & Conditions
            </h1>
      ...
      {/* Main Content Layout */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col lg:flex-row gap-12 lg:gap-24 relative z-10">
        ...
        <div className="hidden lg:block lg:w-1/4 shrink-0 self-start sticky top-[120px] reveal-up">
          <div className="bg-black/20 backdrop-blur-md rounded-2xl p-6 border border-white/5">
            <h3 className="text-xl font-bold mb-6 text-white tracking-wide">Table of Contents</h3>
        ...
        {/* Right Content */}
        <div className="w-full lg:w-3/4 pr-12 lg:pr-0 flex flex-col gap-20 pb-32">
          {termsSections.map((section) => (
            <div key={section.id} id={section.id} className="scroll-mt-36">
              <h2 className="text-2xl md:text-4xl font-semibold mb-8 text-white tracking-tight">
                {section.title}
              </h2>
              <div className="flex flex-col gap-6 text-gray-300 leading-relaxed text-base md:text-lg">
                {section.content.map((paragraph, idx) => {
                  const isSubheading = /^(?:\d+\.\d+|Note:|Eligibility:|Advance Payments:|Milestone Assessment:|Completed Deliverables:|Estimated Targets:|Client Dependencies:|External Limitations:|Ownership Hold:|Transfer of Rights:|Liability Caps:|Damage Types:|Service Basis:|Legal Framework:|Jurisdiction:|Accuracy Hold:|Content Protection:|Acceptable Use:)/.test(paragraph);
                  if (isSubheading) {
                    return (
                      <h3 key={idx} className="text-lg md:text-xl font-semibold text-white mt-4 mb-2 tracking-tight">
                        {paragraph}
                      </h3>
                    );
                  }
                  ...
                  if (colonIndex > 0 && colonIndex < 35 && !paragraph.startsWith('http')) {
                    const prefix = paragraph.substring(0, colonIndex + 1);
                    const rest = paragraph.substring(colonIndex + 1);
                    return (
                      <p key={idx} className="pl-4 border-l-2 border-white/10 hover:border-white/30 transition-colors">
                        <strong className="text-white font-medium">{prefix}</strong>{rest}
                      </p>
                    );
                  }
